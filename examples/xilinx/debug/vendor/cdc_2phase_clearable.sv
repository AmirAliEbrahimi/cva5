// ----------------------------------------------------------------------------
// Local copy of common_cells v1.24.0 src/cdc_2phase_clearable.sv, modified for
// Vivado IP packaging: ipx::package_project -import_files flattens all sources
// into one directory, so `include "common_cells/registers.svh"` cannot resolve.
// The only macro used here (FFNR, plain flop without reset) is expanded inline.
// No functional change. Remove this copy if packaging is changed to keep the
// include tree.
// ----------------------------------------------------------------------------
...
// `include "common_cells/registers.svh" -- removed, see header
